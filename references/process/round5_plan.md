...
Tasks:
1. Upgrade `10_extract_1996.py` with optional OCR fallback path, de-columnization, and per-record confidence flags.
2. Add confidence-aware extraction report metrics and filtering option.
...
- `10_mit_1996_extraction_report.txt` includes confidence and coverage metrics.
...
2. `10_extract_1996.py --use-local`: 2560 records, description non-empty 67.93%, avg confidence 0.611.
