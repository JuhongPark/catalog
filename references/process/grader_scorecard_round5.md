...
- Evidence: 2560 records, description non-empty 67.93%, avg reliability 0.611 with per-record quality indicators.
- Improvement: de-columnization, reliability scoring, optional OCR fallback path, reliability filter support.
...
- Upgraded 1996 extraction quality controls with reliability and filtering signals.
- Added crosswalk-aware interpretation in comparative analyses (items 12 and 14).

Remaining highest-impact gap:
- OCR execution path is runtime-dependent and currently inactive in this environment (`tesseract` unavailable), so low-reliability legacy rows still remain.
